// Puerto de la base educativa de ejercicios + esquema RIR (index.html,
// sección "base educativa + RIR + progresión (Parte B)").
//
// NOTA de organización (deviation documentada, ver task-5-report.md): el plan
// original asigna este archivo a la Task 6 (screen "Hoy"), junto con
// sessionMaxW/progressionWarn (que sí son específicos de Hoy — comparan
// contra el historial de sesiones). Pero Rutina.jsx y dos de sus sheets
// (DayPeek, ExInfo) necesitan `exInfo`/`rirScheme` para su propio markup
// (el botón ⓘ de cada ejercicio y la línea "RIR 3/2/1/0" bajo cada ejercicio
// ya existen en el editor de rutina del original, no sólo en Hoy), así que
...
export const EXDB = [
  ...
  { k: ['kelso shrug', 'kelso', 'shrug', 'encogimiento', 'encogimientos'], m: 'Trapecio medio · romboides · trapecio inferior', w: 'Es un encogimiento hecho en posición de remo (tumbado en banco inclinado o inclinado hacia adelante): el codo NO se flexiona, sólo se retraen y elevan las escápulas. Al sacar el bíceps y el dorsal de la ecuación, el trapecio medio y los romboides —que en el remo suelen ceder el protagonismo— reciben el estímulo directo. Es el accesorio que llena la espalda alta y mejora la retracción escapular en press y remo. Recorrido corto: prioridad a la pausa en contracción, no al peso.' },
