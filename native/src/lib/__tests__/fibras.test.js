// Puerto de web/src/lib/__tests__/fibras.test.js — conversión mecánica
// vi. -> jest. (no hay usos de vi. en este archivo), imports globales de
// Jest (describe/it/expect no se importan explícitamente, mismo patrón que
// el resto de los tests portados en esta etapa). Vive en __tests__/ para
// que ../fibras.js, ../muscle.js y ../bodydata.js resuelvan igual que en
// la PWA (native/src/lib/__tests__/fibras.test.js -> native/src/lib/*.js).
...
describe('las porciones que nombra se pueden pintar', () => {
  ...
  it('todo grupo tiene a qué zona del cuerpo corresponde', () => {
    for (const g of [
      ...
      // Jest, a diferencia de Vitest, no acepta un segundo argumento de
      // mensaje en expect(...) (falla con "Expect takes at most one
      // argument"). Se quita el mensaje `g`; la aserción es idéntica.
      expect(esGrupo(g)).toBe(true);
      expect(ZONA_DE[g]).toBeTruthy();
    }
  });
