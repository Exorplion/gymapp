...
- [ ] **Step 1**: Leer `web/src/lib/alarm.js` completo (192 líneas,
...
- [ ] **Step 2**: Instalar `expo-notifications` y `expo-av` (`npx expo
...
- [ ] **Step 3**: Portar `alarm.js` — adaptar `sonar(texto, alCallar)`/
...
- [ ] **Step 1**: Leer `web/src/lib/rest.js` completo (135 líneas) —
  ya leído en el análisis de esta etapa, releer con foco en el timing
  exacto (`T.end`, `setInterval(tickRest, 250)`).

- [ ] **Step 2**: Portar `rest.js` verbatim en su lógica de cuenta
...
- [ ] **Step 3**: Tests para la lógica de cuenta regresiva pura —
...
- [ ] **Step 1**: Leer `web/src/components/RestTimer.jsx` completo
...
- [ ] **Step 2**: Portar `RestTimer` a `native/src/components/RestTimer.js`
...
- [ ] **Step 3**: Montar `<RestTimer/>` en `App.js`, siempre presente
...
- [ ] Confirmar que `RestTimer` usa `<Defs>` propios dentro de su
  propio `<Svg>` (no compartidos con ningún otro componente) — mismo
  grep que atrapó el bug de Etapa 4a.
- [ ] Confirmar que la notificación local se PROGRAMA para el momento
...
- [ ] Confirmar que `session.js` sigue importando `startRest`/`stopRest`
  de `rest.js` sin cambios en sus imports (la interfaz pública no
  cambió de forma).
- [ ] Confirmar que `recuperarRest()` usa `AppState` de RN, no ningún
